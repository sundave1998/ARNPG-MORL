...
"""Anchor Changing Natural Policy Gradient Method with Softmax Parametrization
"""
...
# Anchor Changing Natural Policy Gradient Method with Softmax Parametrization
def naturalgradient(prob_transition, reward, utility, theta, s, a, rho, gamma, old_prob, alpha, old_vrvals, old_vgvals):
    ...
    d_s_pi = (1 - gamma) * np.linalg.inv(np.identity(s) - gamma * P_theta)  # |S|*|S|
    ...
    V = Vr / old_vrvals + Vg / old_vgvals - alpha / (1 - gamma) * regular_term

    qvals = Q_cal(V, reward / old_vrvals + utility / old_vgvals + alpha * np.log(old_prob))
    vvals = np.zeros(s * a)
    for i in range(s):
        for j in range(a):
            vvals[i * a + j] = V[i]
...
for step in steps:
    for alpha in alphas:
        ...
        for k in range(N):
            ...
            # V(\rho): 1*1
            old_vrvals = np.dot(np.transpose(Vr), rho)
            old_vgvals = np.dot(np.transpose(Vg), rho)
            ...
            for l in range(inner):
                ng = naturalgradient(prob_transition, reward, utility, theta, s, a, rho, gamma, old_prob, alpha, old_vrvals, old_vgvals)
                theta += step * ng

        final_gap.append(gap[-1])

# Saving the data. This can be loaded to make the figure again.
np.savetxt('ARNPG_gap_s20a10g8b3_inner10.txt', gap)
